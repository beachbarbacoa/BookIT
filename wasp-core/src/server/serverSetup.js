...
export const setupServer = (app) => {
  ...
  // Static files are always in wasp-core/.wasp/build/web-app in production
  const clientBuildPath = path.resolve(
    process.cwd(),
    'wasp-core/.wasp/build/web-app'
  );
  
  console.log(`[SERVER] Production static file path: ${clientBuildPath}`);
